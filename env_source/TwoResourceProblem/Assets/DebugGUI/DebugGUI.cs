...
{
    public float offset_x = -200f; // graph y-offset height
    public float offset_y = 450f; // graph y-offset height
    ...
    void DrawGraphs()
    {
        ...
        {
            GUI.Box(new Rect(
                offset_x + Screen.width - graphBlockWidth - graphLabelWidth - 5,
                offset_y + group * graphBlockHeight,
                graphLabelWidth + 5,
                graphHeight
            ), "", boxStyle);
        }
        ...
        foreach (GraphContainer graph in graphDictionary.Values)
        {
            ...
            {
                GUI.Box(new Rect(offset_x + Screen.width - graphWidth, offset_y + graphBlockHeight * graph.group, graphWidth, graphHeight), "", boxStyle);
            }

            graph.Draw(new Rect(offset_x + Screen.width - graphWidth, offset_y + graphBlockHeight * graph.group, graphWidth, graphHeight));
        }

        foreach (int group in graphGroupBoxesDrawn)
        {
            float groupOrigin = offset_y + group * graphBlockHeight;
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    GUI.Label(new Rect(offset_x + Screen.width - graphWidth - minMaxXOffset, groupOrigin, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    // Min
                    labelGuiContent.text = graph.min.ToString("F2");
                    GUI.Label(new Rect(offset_x + Screen.width - graphWidth - minMaxXOffset, groupOrigin + graphHeight - textSize.y, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    ...
                    GUI.Label(new Rect(offset_x + Screen.width - graphWidth - xOffset, yOffset, xOffset, graphHeight), labelGuiContent);
