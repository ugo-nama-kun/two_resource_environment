...
def test_get_single_sample():
    ...
    for i in range(10):
        obs = Observation(image=torch.tensor([i]), vector=torch.tensor([i]))
        next_obs = Observation(image=torch.tensor([i+1]), vector=torch.tensor([i+1]))
        buf.append(observation=obs, action=i, next_observation=next_obs)

    for i in range(5):
        assert buf.get_single_experience(time_step=i).observation.image == torch.tensor([i])
        ...
        assert buf.get_single_experience(time_step=i).next_observation.image == torch.tensor([i+1])
